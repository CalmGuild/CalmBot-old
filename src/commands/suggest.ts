...
module.exports = {
  ...
  run: async function run(client: Client, message: Message, args: Array<String>) {
    ...
    const suggestionEmbed = new MessageEmbed().setFooter(`${message.member.displayName}`, message.author.displayAvatarURL() + `CalmBot v${client.version}`).setColor("#007FFF").setTitle("Suggestion:").setDescription(suggestion).setTimestamp();
